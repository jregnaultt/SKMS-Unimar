...
<aside class="fixed inset-y-0 left-0 z-45 w-64 bg-white border-r border-slate-200/60 text-slate-700 flex flex-col justify-between shrink-0 h-screen transition-transform duration-300 ease-in-out transform lg:translate-x-0 lg:sticky lg:top-0 shadow-sm"
    ...
    <div class="p-6 border-b border-slate-100 shrink-0 relative">
        ...
        <div class="flex items-center space-x-3">
            <!-- Logo SKMS -->
            <div class="p-1.5 bg-[#0d4d98]/5 rounded-lg border border-[#0d4d98]/10">
                <img src="{{ asset('images/logo.svg') }}" alt="SKMS Unimar" class="w-11 h-11 object-contain">
            </div>
            <div>
    ...
    <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
        ...
        @if(auth()->user()->hasAnyRole(['Tutor', 'Jurado', 'Coordinador', 'Decano']))
            <a href="{{ route('assigned-productions.index') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-150 {{ request()->routeIs('assigned-productions.*') ? 'bg-[#0d4d98] text-white font-bold shadow-md shadow-[#0d4d98]/10' : 'text-slate-600 hover:bg-[#0d4d98] hover:text-white hover:shadow-sm' }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
                </svg>
                <span class="text-base">Trabajos Asignados</span>
            </a>
        @endif
        ...
        <a href="{{ route('profile.edit') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-150 {{ request()->routeIs('profile.*') ? 'bg-[#0d4d98] text-white font-bold shadow-md shadow-[#0d4d98]/10' : 'text-slate-600 hover:bg-[#0d4d98] hover:text-white hover:shadow-sm' }}">
            ...
            <span class="text-base">Mi Perfil</span>
        </a>
            ...
            <div x-data="{ adminOpen: {{ request()->routeIs('admin.*') ? 'true' : 'false' }} }" class="space-y-1">
                ...
                <button @click="adminOpen = !adminOpen" 
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <span class="text-base">Panel Admin</span>
                    </div>
                ...
                <div x-show="adminOpen" 
                    ...
                    <a href="{{ route('admin.programs.index') }}" 
                       class="flex items-center space-x-2.5 px-4 py-2 rounded-lg text-sm font-semibold transition {{ Request::routeIs('admin.programs.*') || Request::routeIs('admin.lines.*') || Request::routeIs('admin.periods.*') ? 'text-[#0d4d98] bg-[#0d4d98]/5 font-bold' : 'text-slate-500 hover:text-[#0d4d98] hover:bg-[#0d4d98]/5' }}">
                        <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                        <span>Config. Académica</span>
                    </a>

                    <a href="{{ route('admin.users.index') }}" 
                       class="flex items-center space-x-2.5 px-4 py-2 rounded-lg text-sm font-semibold transition {{ Request::routeIs('admin.users.*') ? 'text-[#0d4d98] bg-[#0d4d98]/5 font-bold' : 'text-slate-500 hover:text-[#0d4d98] hover:bg-[#0d4d98]/5' }}">
                        <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                    ...
                    <a href="{{ route('admin.audit-logs.index') }}" 
                       class="flex items-center space-x-2.5 px-4 py-2 rounded-lg text-sm font-semibold transition {{ Request::routeIs('admin.audit-logs.*') ? 'text-[#0d4d98] bg-[#0d4d98]/5 font-bold' : 'text-slate-500 hover:text-[#0d4d98] hover:bg-[#0d4d98]/5' }}">
                        <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                        <span>Bitácora de Auditoría</span>
                    </a>

                    <a href="{{ route('admin.reports.index') }}" 
                       class="flex items-center space-x-2.5 px-4 py-2 rounded-lg text-sm font-semibold transition {{ Request::routeIs('admin.reports.*') ? 'text-[#0d4d98] bg-[#0d4d98]/5 font-bold' : 'text-slate-500 hover:text-[#0d4d98] hover:bg-[#0d4d98]/5' }}">
                        <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
    ...
    <div class="p-4 border-t border-slate-100 bg-slate-50 shrink-0">
        <div class="flex items-center space-x-3 mb-3">
            <div class="w-9 h-9 rounded-full bg-[#F5B800] text-slate-900 font-bold flex items-center justify-center text-base shadow-inner shrink-0">
