    ...
    <div class="py-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200/60 pb-5">
            <div>
                ...
                <p class="text-sm text-slate-400 font-semibold uppercase tracking-wider mt-0.5">
                ...
                <aside class="w-full lg:w-1/4 shrink-0">
                    <details class="group bg-white border border-slate-200 rounded-2xl shadow-[0_10px_30px_rgba(13,77,152,0.02)] overflow-hidden lg:border-none lg:shadow-none lg:bg-transparent" {{ request()->anyFilled(['program', 'line', 'year', 'type', 'tutor']) ? 'open' : '' }}>
                        <summary class="lg:hidden flex items-center justify-between p-4 cursor-pointer font-bold text-base text-slate-800 uppercase tracking-wider select-none focus:outline-none focus:ring-2 focus:ring-[#0d4d98]/20">
                            ...
                            <svg aria-hidden="true" class="w-4 h-4 text-slate-650 transition-transform duration-200 group-open:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" />
                            </svg>
                        </summary>

                        <div class="hidden lg:!block border-t border-slate-100 lg:border-none p-4 md:p-5 space-y-5 bg-white lg:border lg:border-slate-200 lg:rounded-2xl lg:shadow-[0_10px_30px_rgba(13,77,152,0.02)] group-open:block">
                    ...
                    <div class="p-5 bg-gradient-to-br from-[#0d4d98]/5 to-transparent border border-slate-200/50 rounded-2xl text-center space-y-3 mt-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <p class="text-sm text-slate-600 leading-normal">Toda la producción científica de UNIMAR está catalogada bajo el estándar internacional Dublin Core y es compatible con el protocolo de interoperabilidad OAI-PMH.</p>
                        </div>
                    </div>
                </aside>
                ...
                <section class="w-full lg:w-3/4 space-y-6">
                    ...
                    <div x-data="catalogSearch()" class="bg-white border border-slate-200 rounded-2xl p-4 shadow-[0_10px_30px_rgba(13,77,152,0.02)] relative">
                        <div class="relative flex items-center">
                            ...
                            <input type="text" id="search-input" name="q" x-model="query" @blur="closeDropdown()" @focus="if (query.trim().length >= 3) showDropdown = true" value="{{ request('q') }}" placeholder="Buscar por título, resumen, autores o palabras clave..." aria-label="Buscar publicaciones científicas" class="block w-full ps-11 pe-28 py-3.5 text-sm border-slate-250 rounded-xl bg-slate-50 text-slate-800 focus:ring-[#0d4d98] focus:border-[#0d4d98] placeholder-slate-400">
                        ...
                        <div x-show="showDropdown && results.length > 0" class="absolute left-4 right-4 top-full mt-2 bg-white border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden divide-y divide-slate-100 max-h-60 overflow-y-auto" x-cloak>
                            <template x-for="item in results" :key="item.id">
                                <a :href="'/productions/' + item.uuid" class="block p-3 hover:bg-slate-50 transition text-left">
                    ...
                    <!-- Results Statistics -->
                    <div class="flex items-center justify-between text-sm font-bold text-slate-550 uppercase tracking-wider px-1">
                        <div>
                            @if(request('q'))
                            ...
                            <article class="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-[0_10px_30px_rgba(13,77,152,0.02)] hover:-translate-y-0.5 hover:shadow-md transition-all duration-200 flex flex-col justify-between">
                                <div>
                                    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3 border-b border-slate-100 pb-3">
                                        <span class="px-2.5 py-0.5 text-sm font-extrabold rounded-full bg-[#0d4d98]/10 text-[#0d4d98] border border-[#0d4d98]/20 uppercase tracking-wider self-start">
                                            {{ $production->productionType->name ?? 'Trabajo Científico' }}
                                        </span>
                                        <span class="text-sm text-slate-550 font-bold uppercase tracking-wider">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-slate-600 border-t border-slate-100 pt-4 mb-4">
                                    ...
                                    @if($production->keywords && $production->keywords->isNotEmpty())
                                        <div class="flex flex-wrap gap-1.5 mb-4">
                                            @foreach($production->keywords as $keyword)
                                                <span class="px-2 py-0.5 bg-slate-100 text-slate-550 text-xs rounded-md font-bold font-mono uppercase">
                                                    #{{ $keyword->name }}
                                                </span>
                                            @endforeach
                                        </div>
