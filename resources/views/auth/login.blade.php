<x-guest-layout>
    ...
    <div class="text-center mb-8">
        ...
        <p class="text-sm text-slate-500 mt-1.5 font-medium">Ingresa tus credenciales académicas para acceder</p>
    </div>

    <!-- Estado de la Sesión -->
    <x-auth-session-status class="mb-6 p-3 bg-blue-50 border-l-4 border-unimar-blue rounded-r-lg text-sm font-semibold text-unimar-blue" :status="session('status')" />
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div x-data="{ showPassword: false }">
            <label for="password" class="block text-sm font-bold text-slate-600 uppercase tracking-wider mb-1.5">Contraseña</label>
            <div class="relative">
                <input id="password" 
        ...
        <div class="flex items-center justify-between pt-1">
            <label for="remember_me" class="inline-flex items-center cursor-pointer">
                ...
                <span class="ms-2 text-sm text-slate-600 font-medium select-none">Recordarme</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-sm text-unimar-blue hover:text-unimar-blue/80 hover:underline font-semibold transition" href="{{ route('password.request') }}">
