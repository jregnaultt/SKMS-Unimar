                    ...
                    <div class="p-6">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                    Administra las líneas de investigación asociadas a cada programa académico.
                                </p>
                            </div>
                            <a href="{{ route('admin.lines.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-indigo-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 shadow-sm self-start">
                        ...
                        @if ($lines->isEmpty())
                            <div class="text-center py-12 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                                ...
                                <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-gray-100">No hay líneas de investigación registradas</h3>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Comienza creando una línea de investigación.</p>
                            </div>
                        @else
                                ...
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                                        <tr>
                                            ...
                                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Programa Académico</th>
                                        ...
                                        @foreach ($lines as $line)
                                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors duration-150">
                                                <td class="px-6 py-4">
                                                    ...
                                                    @if($line->description)
                                                        <div class="text-xs text-gray-500 dark:text-gray-400 line-clamp-1 mt-0.5" title="{{ $line->description }}">
                                                            {{ $line->description }}
                                                        </div>
                                                    @endif
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    @if($line->academicProgram)
                                                        <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-md bg-indigo-50 text-indigo-700 dark:bg-indigo-950/30 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/50">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $line->is_active ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-400' : 'bg-rose-100 text-rose-800 dark:bg-rose-950/30 dark:text-rose-400' }}">
                                                        {{ $line->is_active ? 'Activo' : 'Inactivo' }}
                                                    </span>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div class="flex items-center justify-end space-x-3">
                                                        <a href="{{ route('admin.lines.edit', $line) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 font-semibold">
                                                            Editar
                                                        </a>
