...
<x-dashboard-layout :roles="$userRoles" :activeRole="$activeRole">
    <div class="space-y-8 max-w-7xl mx-auto pb-12">
        ...
        <div class="border-b border-slate-200 bg-white p-4 rounded-2xl shadow-sm border flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <!-- Botón dinámico -->
            <a href="{{ route('admin.programs.create') }}" class="py-2.5 px-5 bg-unimar-blue hover:bg-unimar-blue/95 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition shadow-sm hover:shadow-md shrink-0 inline-flex items-center">
        ...
        <div class="bg-white border border-slate-200/80 shadow-sm rounded-2xl overflow-hidden">
            <div class="p-6 border-b border-slate-100">
                ...
                <p class="text-xs text-slate-500 mt-0.5 font-medium">Administra las diferentes carreras o postgrados asociados al Decanato de Ingeniería</p>
            </div>
            ...
            @if ($programs->isEmpty())
                <div class="py-16 text-center text-slate-400 border-t border-slate-100">
                    ...
                    <h4 class="text-sm font-semibold text-slate-700">No hay programas académicos registrados</h4>
                    <p class="text-xs text-slate-500 mt-1">Comienza creando un programa académico utilizando el botón de arriba.</p>
                </div>
            @else
                                ...
                                <tr class="hover:bg-slate-50/50 transition duration-150">
                                    ...
                                    <td class="p-4">
                                        ...
                                        @if($program->description)
                                            <p class="text-xs text-slate-400 font-medium line-clamp-1 mt-0.5" title="{{ $program->description }}">
                                                {{ $program->description }}
                                            </p>
                                        @endif
                                    </td>
                                    <td class="p-4 whitespace-nowrap">
                                        <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-bold rounded-full {{ $program->is_active ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-rose-50 text-rose-800 border border-rose-200' }}">
                                            {{ $program->is_active ? 'Activo' : 'Inactivo' }}
                                        </span>
                                    </td>
                                    <td class="p-4 pr-6 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-3">
                                            <a href="{{ route('admin.programs.edit', $program) }}" class="text-unimar-blue hover:text-unimar-blue/80 font-bold text-xs uppercase tracking-wider transition">
                                                Editar
                                            </a>
