...
<x-dashboard-layout :roles="$userRoles" :activeRole="$activeRole">
    <div class="space-y-8 max-w-8xl mx-auto pb-12">
        ...
        <div class="border-b border-slate-200 bg-white p-4 rounded-2xl shadow-sm border flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <!-- Botón dinámico -->
            <a href="{{ route('admin.programs.create') }}" class="py-3 px-5 bg-unimar-blue hover:bg-unimar-blue/95 text-white font-bold rounded-xl text-base uppercase tracking-wider transition shadow-sm hover:shadow-md shrink-0 inline-flex items-center h-11 cursor-pointer">
        ...
        <div class="bg-white border border-slate-200/80 shadow-sm rounded-2xl overflow-hidden">
            <div class="p-6 border-b border-slate-100">
                <h3 class="text-lg font-bold text-slate-800 font-sans">Listado de Programas Académicos</h3>
                <p class="text-base text-slate-550 mt-0.5 font-bold uppercase tracking-wider">Administra las diferentes carreras o postgrados asociados al Decanato de Ingeniería</p>
            </div>
            ...
            @if ($programs->isEmpty())
                <div class="py-16 text-center text-slate-550 border-t border-slate-100">
                    ...
                    <h4 class="text-base font-bold text-slate-700 uppercase tracking-wider">No hay programas académicos registrados</h4>
                    <p class="text-base text-slate-550 mt-1 font-bold">Comienza creando un programa académico utilizando el botón de arriba.</p>
                </div>
            @else
                    ...
                    <table class="w-full text-left border-collapse min-w-[650px]">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-200 text-base font-bold uppercase tracking-wider text-slate-650">
                                ...
                                <th class="p-4">Nombre del Programa</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-base text-slate-700">
                            @foreach ($programs as $program)
                                <tr class="hover:bg-slate-50/50 transition duration-150">
                                    <td class="p-4 pl-6 whitespace-nowrap font-mono font-bold text-unimar-blue">
                                        {{ $program->code }}
                                    </td>
                                    <td class="p-4">
                                        ...
                                        @if($program->description)
                                            <p class="text-base text-slate-550 font-bold line-clamp-1 mt-0.5" title="{{ $program->description }}">
                                                {{ $program->description }}
                                            </p>
                                        @endif
                                    </td>
                                    <td class="p-4 whitespace-nowrap">
                                        <span class="px-2.5 py-1 inline-flex text-base leading-5 font-bold rounded-full {{ $program->is_active ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-rose-50 text-rose-800 border border-rose-200' }}">
                                            {{ $program->is_active ? 'Activo' : 'Inactivo' }}
                                        </span>
                                    </td>
                                    <td class="p-4 pr-6 whitespace-nowrap text-right text-base font-medium">
                                        <div class="flex items-center justify-end space-x-3">
                                            <a href="{{ route('admin.programs.edit', $program) }}" class="px-3.5 py-2 border border-slate-200 hover:bg-slate-50 hover:border-slate-300 text-slate-700 rounded-lg text-base font-bold transition shadow-sm h-11 inline-flex items-center cursor-pointer">
                                                Editar
                                            </a>
