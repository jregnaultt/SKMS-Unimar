...
<x-dashboard-layout :roles="$userRoles" :activeRole="$activeRole">
    <div class="space-y-8 max-w-8xl mx-auto pb-12">
        ...
        <div id="admin-config-tabs" class="border-b border-slate-200 bg-white p-4 rounded-2xl shadow-sm border flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <!-- Botón dinámico -->
            <a href="{{ route('admin.periods.create') }}" class="py-3 px-5 bg-unimar-blue hover:bg-unimar-blue/95 text-white font-bold rounded-xl text-base uppercase tracking-wider transition shadow-sm hover:shadow-md shrink-0 inline-flex items-center h-11 cursor-pointer">
        ...
        <div id="admin-config-table-card" class="bg-white border border-slate-200/80 shadow-sm rounded-2xl overflow-hidden">
            ...
            @if ($periods->isEmpty())
                <div class="py-16 text-center text-slate-550 border-t border-slate-100">
                    ...
                    <h4 class="text-base font-bold text-slate-700 uppercase tracking-wider">No hay períodos académicos registrados</h4>
                    <p class="text-base text-slate-555 mt-1 font-bold">Comienza creando un período académico utilizando el botón de arriba.</p>
                </div>
            @else
                    ...
                    <table class="w-full text-left border-collapse min-w-[650px]">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-200 text-base font-bold uppercase tracking-wider text-slate-650">
                                <th class="p-4 pl-6">Nombre del Período</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-base text-slate-700">
                            @foreach ($periods as $period)
                                <tr class="hover:bg-slate-50/50 transition duration-150">
                                    <td class="p-4 pl-6 whitespace-nowrap font-bold text-slate-800">
                                        {{ $period->name }}
                                    </td>
                                    <td class="p-4 whitespace-nowrap text-slate-700 font-semibold">
                                    ...
                                    <td class="p-4 whitespace-nowrap">
                                        <span class="px-2.5 py-1 inline-flex text-base leading-5 font-bold rounded-full {{ $period->is_active ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-rose-50 text-rose-800 border border-rose-200' }}">
                                            {{ $period->is_active ? 'Activo' : 'Inactivo' }}
                                        </span>
                                    </td>
                                    <td class="p-4 pr-6 whitespace-nowrap text-right text-base font-medium">
                                        <div class="flex items-center justify-end space-x-3">
                                            <a href="{{ route('admin.periods.edit', $period) }}" class="px-3.5 py-2 border border-slate-200 hover:bg-slate-50 hover:border-slate-300 text-slate-700 rounded-lg text-base font-bold transition shadow-sm h-11 inline-flex items-center cursor-pointer">
                                                Editar
                                            </a>
