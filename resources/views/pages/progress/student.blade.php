        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-5">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-base text-slate-500 mt-1 font-medium max-w-2xl truncate" title="{{ $production->title }}">
                        {{ $production->title }}
                    </p>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div id="milestones-progress-card" class="bg-white border border-slate-200/80 shadow-sm rounded-2xl p-4 md:p-5 flex flex-col sm:flex-row items-center gap-4">
                    ...
                    <div class="text-center sm:text-left space-y-2">
                        ...
                        <div class="inline-flex items-center px-3 py-1 bg-slate-50 rounded-lg border border-slate-200 text-sm font-bold text-slate-600 mt-1">
                ...
                <div id="milestones-timeline-card" class="bg-white border border-slate-200/80 shadow-sm rounded-2xl p-4 md:p-5 sm:p-8 space-y-6" x-data="{ activeMilestone: 0 }">
                    ...
                    @if ($milestones->isEmpty())
                        <div class="py-12 text-center text-slate-400 border border-dashed border-slate-200 rounded-xl">
                            ...
                            <span class="text-base font-medium">No hay hitos académicos programados para esta producción.</span>
                        </div>
                    @else
                        <!-- Stepper Timeline -->
                            ...
                            @foreach ($milestones as $milestone)
                                ...
                                <div class="relative pl-8 group">
                                    ...
                                    <button type="button" 
                                            ...
                                            aria-label="Ver detalles de hito {{ $milestone->title }}"
                                            class="absolute -left-[19px] top-1.5 flex items-center justify-center w-9 h-9 rounded-full border-2 {{ $nodeBorder }} {{ $nodeBg }} shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#0d4d98]/50 shrink-0 z-10">
                                        @if ($isCompleted)
                                            <svg aria-hidden="true" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3">
                                    ...
                                    <div class="cursor-pointer" @click="activeMilestone = (activeMilestone === {{ $loop->index }} ? null : {{ $loop->index }})">
                                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                            <div>
                                                <h4 class="font-bold text-slate-800 text-base group-hover:text-unimar-blue transition duration-150">
                                                    {{ $milestone->title }}
                                                </h4>
                                                <span class="text-sm uppercase font-bold text-slate-550 tracking-wider">
                                            ...
                                            <div class="flex items-center space-x-2">
                                                <span class="inline-flex items-center px-2 py-0.5 rounded text-sm font-bold uppercase tracking-wider border {{ $badgeClass }}">
                                                    {{ $badgeLabel }}
                                                </span>
                                                <!-- Icono indicador de expansión -->
                                                <svg aria-hidden="true" class="w-4 h-4 text-slate-550 transition-transform duration-200" :class="{'rotate-180': activeMilestone === {{ $loop->index }}}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div x-show="activeMilestone === {{ $loop->index }}"
                                         ...
                                         style="display: none;"
                                         class="mt-3">
                                        <div class="p-4 bg-slate-50 border border-slate-200/80 rounded-xl space-y-3 text-sm">
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                <div>
                                                    <span class="block text-slate-550 font-bold uppercase tracking-wider text-sm">Fecha Límite Establecida</span>
                                                    <span class="font-bold text-slate-700 text-base">{{ $milestone->scheduled_date ? $milestone->scheduled_date->format('d/m/Y') : 'Sin fecha límite' }}</span>
                                                </div>
                                                @if ($milestone->completed_date)
                                                    <div>
                                                        <span class="block text-slate-550 font-bold uppercase tracking-wider text-sm">Fecha de Cumplimiento</span>
                                                        <span class="font-bold text-emerald-600 text-base">{{ $milestone->completed_date->format('d/m/Y') }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                            ...
                                            @if (!$isCompleted && $milestone->type === 'delivery' && $production->workflow_state === 'needs_corrections')
                                                <div class="border-t border-slate-200 pt-3 space-y-2">
                                                    <span class="block text-slate-550 font-bold uppercase tracking-wider text-sm">Observaciones Pendientes de Corrección</span>
                                                    ...
                                                    <div class="pt-1">
                                                        <a href="{{ route('productions.show', $production) }}" class="inline-flex items-center py-2.5 px-4 bg-unimar-blue hover:bg-unimar-blue/95 text-white font-bold rounded-lg text-sm uppercase tracking-wider transition shadow-sm h-11">
                    ...
                    @else
                        <div class="space-y-3 max-h-[280px] overflow-y-auto pr-1">
                            @foreach ($version_history as $version)
                                <div class="p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm space-y-2">
