...
<x-dashboard-layout :roles="$roles" :activeRole="$activeRole">
    ...
    <div class="py-6 space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200/60 pb-5">
            <div>
                <a href="{{ route('dashboard') }}" class="inline-flex items-center text-base font-bold text-slate-555 hover:text-[#0d4d98] mb-2 transition duration-150 uppercase tracking-wider">
                    <svg aria-hidden="true" class="w-3.5 h-3.5 mr-1.5 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                ...
                <p class="text-base text-slate-550 font-bold uppercase tracking-wider mt-0.5">Indicadores de productividad y rendimiento científico</p>
            </div>
            <div>
                <a href="{{ route('admin.reports.index') }}" class="inline-flex items-center px-4 py-3 bg-[#0d4d98] hover:bg-blue-800 text-white rounded-xl text-base font-bold uppercase tracking-wider transition shadow-sm hover:shadow-md h-11 cursor-pointer">
                    <svg aria-hidden="true" class="w-4 h-4 mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div id="bibliometrics-line-chart" class="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-[0_10px_30px_rgba(13,77,152,0.03)] border-t-4 border-t-[#0d4d98] space-y-4">
            ...
            @if(empty($metrics['yearly_evolution']))
                <div class="flex flex-col items-center justify-center p-12 text-center">
                    ...
                    <p class="text-base text-slate-550 font-bold italic">No hay suficientes datos históricos para graficar la evolución temporal.</p>
                </div>
            @else
        ...
        <div id="bibliometrics-comparisons" class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-[0_10px_30px_rgba(13,77,152,0.03)] border-t-4 border-t-[#0d4d98] flex flex-col space-y-4"
                ...
                <div class="flex items-center justify-between border-b border-slate-100 pb-3 flex-wrap gap-2">
                    <div>
                        <h3 class="text-base font-bold text-slate-850 uppercase tracking-wider">Comparativa de Producción</h3>
                        <p class="text-base text-slate-550 font-bold uppercase tracking-wider mt-0.5">Distribución cuantitativa de las obras</p>
                    </div>
                    ...
                    <div class="inline-flex rounded-lg shadow-sm bg-slate-50 p-1 border border-slate-200/40">
                        <button type="button"
                                ...
                                :class="activeTab === 'program' ? 'bg-white text-[#0d4d98] font-bold shadow-sm' : 'text-slate-500 hover:text-slate-650'"
                                class="px-3 py-2 rounded-md text-base uppercase font-bold tracking-wider transition cursor-pointer">
                            Por Programa
                        </button>
                        <button type="button"
                                @click="activeTab = 'line'"
                                :class="activeTab === 'line' ? 'bg-white text-[#0d4d98] font-bold shadow-sm' : 'text-slate-500 hover:text-slate-650'"
                                class="px-3 py-2 rounded-md text-base uppercase font-bold tracking-wider transition cursor-pointer">
                            Por Línea
                        </button>
                    </div>
            ...
            <div class="space-y-4">
                <!-- Top Tutors Table -->
                <div class="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-[0_10px_30px_rgba(13,77,152,0.03)] border-t-4 border-t-[#F5B800] space-y-3">
                    <div class="border-b border-slate-100 pb-2">
                        <h3 class="text-base font-bold text-slate-850 uppercase tracking-wider">Top Tutores</h3>
                        <p class="text-base text-slate-550 font-bold uppercase tracking-wider mt-0.5">Docentes con mayor volumen de tutorías aprobadas</p>
                    </div>

                    <div class="overflow-x-auto rounded-xl border border-slate-100">
                        <table class="min-w-full divide-y divide-slate-100 text-base">
                            <thead class="bg-slate-50">
                                <tr>
                                    <th class="px-4 py-2.5 text-left font-bold text-slate-650 uppercase tracking-wider">Tutor Académico</th>
                                    <th class="px-4 py-2.5 text-right font-bold text-slate-650 uppercase tracking-wider w-24">Prods.</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100 bg-white">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="2" class="px-4 py-4 text-base text-slate-550 text-center italic font-semibold">No hay datos disponibles para tutores.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-[0_10px_30px_rgba(13,77,152,0.03)] border-t-4 border-t-[#F5B800] space-y-3">
                    <div class="border-b border-slate-100 pb-2">
                        <h3 class="text-base font-bold text-slate-850 uppercase tracking-wider">Líneas de Mayor Impacto</h3>
                        <p class="text-base text-slate-550 font-bold uppercase tracking-wider mt-0.5">Líneas de investigación con mayor nivel de publicación</p>
                    </div>
                    ...
                    <div class="overflow-x-auto rounded-xl border border-slate-100">
                        <table class="min-w-full divide-y divide-slate-100 text-base">
                            <thead class="bg-slate-50">
                                <tr>
                                    <th class="px-4 py-2.5 text-left font-bold text-slate-650 uppercase tracking-wider">Línea de Investigación</th>
                                    <th class="px-4 py-2.5 text-right font-bold text-slate-650 uppercase tracking-wider w-24">Prods.</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100 bg-white">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="2" class="px-4 py-4 text-base text-slate-550 text-center italic font-semibold">No hay datos disponibles para líneas.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
